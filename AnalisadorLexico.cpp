...
bool listadeDeclaracoes(std::string txt) {
  ...
  if(txt.compare(0,6,"matriz") == 0) {
    ...
    if(txt.find("como") < txt.find("[") && txt.find("como") != -1) {
      ...
      txt.erase(0,1);
      if(txt.size() > 0) {
        return tipo(t) && identificador(id1) && identificador(id2) && valorInteiro(v1) && valorInteiro(v2) && listadeDeclaracoes(txt);
      }
      return tipo(t) && identificador(id1) && identificador(id2) && valorInteiro(v1) && valorInteiro(v2);
    }
